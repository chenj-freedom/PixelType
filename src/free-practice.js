export const FREE_PRACTICE_POOLS = {
  letters: [
...
export const FREE_PRACTICE_MODES = [
  {
    id: 'letters',
...
export function buildFreePracticeMission(modeId = 'mixed', random = Math.random) {
...
export function getFreePracticeMode(modeId) {
  return FREE_PRACTICE_MODES.find((mode) => mode.id === modeId)
    || FREE_PRACTICE_MODES.find((mode) => mode.id === 'mixed');
}

export function pickRandomTargets(pool, count, random = Math.random) {
  const source = [...pool];
  const result = [];
